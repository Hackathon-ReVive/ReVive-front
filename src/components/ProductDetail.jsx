        ...
        <>
            <section className="container bg-green-100 mx-auto p-4">
                ...
                <div className="flex flex-col items-center">
                    <img src={product.image} alt={product.name} className="w-full h-40 object-cover rounded-lg aspect-[2/3]" />
                    <p className="font-['Fira_Sans'] text-black">{product.type}</p>
                    <p className="font-['Fira_Sans'] text-emerald-900 font-bold">{product.price} €</p>
                    <p className="font-['Fira_Sans'] text-black mt-1">{product.description}</p>
                    <button onClick={() => addToCart(product)} className="mt-2 bg-emerald-900 text-white py-1 px-4 rounded">
                        Agregar al carrito
                    </button>
                </div>
            </section>
